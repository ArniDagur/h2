...
#[tokio::test]
async fn request_asterisk_path_non_options_is_user_error() {
    h2_support::trace_init!();
    let (io, mut srv) = mock::new();
    ...
    let srv = async move {
        ...
        .await;
        // OPTIONS with path * — server may not care about exact path form here;
        // just ensure the rejected GET * did not burn stream 1 incorrectly.
        // Actually after UserError the next valid request uses stream 1.
        srv.send_frame(frames::headers(1).response(200).eos()).await;
    };
    ...
    let h2 = async move {
        ...
        // GET with path * via URI path_and_query
        ...
        client
            .send_request(request, true)
            .expect_err("GET * must be UserError");
...
/// TE: Trailers (mixed case) is legal — transfer-coding is case-insensitive
/// (RFC 9110). Pre-fix only accepted exact "trailers".
#[tokio::test]
async fn request_te_trailers_case_insensitive() {
    h2_support::trace_init!();
    let (io, mut srv) = mock::new();
    ...
    let client = async move {
        ...
        let (resp, _) = client
            .send_request(request, true)
            .expect("TE: Trailers must be allowed");
...
/// Empty IPv6 literal authority `[]` must not be generated (RFC 3986 §3.2.2).
#[tokio::test]
async fn request_with_empty_ipv6_literal_authority_is_user_error() {
    h2_support::trace_init!();
    let (io, mut srv) = mock::new();
    ...
    let client = async move {
        let (mut client, mut conn) = client::handshake(io).await.unwrap();
        // http::Uri accepts https://[]/ with host "[]".
        let request = Request::builder()
            .version(Version::HTTP_2)
            .uri("https://[]/")
            .body(())
        ...
        client
            .send_request(request, true)
            .expect_err("empty IPv6 literal authority must be UserError");
        ...
        client
            .send_request(request, true)
            .expect_err("empty IPv6 literal with port must be UserError");
...
/// URI forms like `https://example.com?q=1` must emit `:path: /?q=1`, not `?q=1`.
#[tokio::test]
async fn query_only_uri_sends_slash_query_path() {
    h2_support::trace_init!();
    let (io, mut srv) = mock::new();
...
/// (receive already rejects — F39).
#[tokio::test]
async fn send_request_rejects_mismatched_content_length() {
    h2_support::trace_init!();
    let (io, mut srv) = mock::new();
