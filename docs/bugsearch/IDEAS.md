...
## Tried
- F1–F108 fixes; #853 dismiss; I1/I2 conservation; S3 dismiss.
...
- F5 residual: `wait_open` / `wait_send` remain separate. No SendRequest path writes `send_task`.
- I1 vs `max_concurrent_reset_streams(0)` + F81: `is_closed()` is false while RST/body sit in `pending_send` or `buffered>0`, so `transition_after` does not unlink a stream that still holds send `available`. Capacity returns on RST pop `reclaim_all`. Not a conservation panic.
